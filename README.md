...
Build artifacts (mainly `rust/target/` and `haskell/dist-newstyle/`) can consume 250+ MB. To reclaim that space:
...
Removes compiled binaries, build caches, `results/*.json`, and `index.html`. Everything is regenerated by running `./run_all.sh`.
