...
_PYTHON_PATH = ("M14.25.18l.9.2.73.26.59.3.45.32.34.34.25.34.16.33.1.3.04.26.02.2-.01.13V8.5l-.05.63-.13.55-.21.46"
                ...
                "-.36-.36-.35-.46-.32-.59-.28-.73-.21-.88-.14-1.05-.05-1.23.06-1.22.16-1.04.24-.87.32-.71.36-.57.4"
                "-.44.42-.33.42-.24.4-.16.36-.1.32-.05.24-.01h.16l.06.01h8.16v-.83H6.18l-.01-2.75-.02-.37.05-.34.11"
                "-.31.17-.28.25-.26.31-.23.38-.2.44-.18.51-.15.58-.12.64-.1.71-.06.77-.04.84-.02 1.27.05zm-6.3 1.98l"
                "-.23.33-.08.41.08.41.23.34.33.22.41.09.41-.09.33-.22.23-.34.08-.41-.08-.41-.23-.33-.33-.22-.41-.09"
                "-.41.09zm13.09 3.95l.28.06.32.12.35.18.36.27.36.35.35.47.32.59.28.73.21.88.14 1.04.05 1.23-.06 1.23"
                "-.16 1.04-.24.86-.32.71-.36.57-.4.45-.42.33-.42.24-.4.16-.36.09-.32.05-.24.02-.16-.01h-8.22v.82h5.84l"
                ".01 2.76.02.36-.05.34-.11.31-.17.29-.25.25-.31.24-.38.2-.44.17-.51.15-.58.13-.64.09-.71.07-.77.04"
                "-.84.01-1.27-.04-1.07-.14-.9-.2-.73-.25-.59-.3-.45-.33-.34-.34-.25-.34-.16-.33-.1-.3-.04-.25-.02-.2"
                ".01-.13v-5.34l.05-.64.13-.54.21-.46.26-.38.3-.32.33-.24.35-.2.35-.14.33-.1.3-.06.26-.04.21-.02.13"
                "-.01h5.84l.69-.05.59-.14.5-.21.41-.28.33-.32.27-.35.2-.36.15-.36.1-.35.07-.32.04-.28.02-.21V6.07h2.09l"
                ".14.01zm-6.47 14.25l-.23.33-.08.41.08.41.23.33.33.23.41.08.41-.08.33-.23.23-.33.08-.41-.08-.41-.23"
                "-.33-.33-.23-.41-.08-.41.08z")
...
_POLARS_SVG = (
    ...
    '<circle cx="12" cy="12" r="7" fill="white" opacity="0.65"/>'
...
_OCTAVE_SVG = (
    '<svg class="lang-svg" viewBox="0 0 24 24" '
    'xmlns="http://www.w3.org/2000/svg" aria-hidden="true" fill="currentColor">'
    '<rect x="2" y="3" width="2.5" height="18"/>'
    '<rect x="2" y="3" width="6" height="2.5"/>'
    '<rect x="2" y="18.5" width="6" height="2.5"/>'
    '<rect x="19.5" y="3" width="2.5" height="18"/>'
    '<rect x="16" y="3" width="6" height="2.5"/>'
    '<rect x="16" y="18.5" width="6" height="2.5"/>'
...
LANG_META = {
    "Python (NumPy)":  {"color": "#3776AB", "bg": "#EBF3FB", "chart_label": "🐍",
                        "logo": _svg(_PYTHON_PATH)},
    "Python (Pandas)": {"color": "#150458", "bg": "#EDEBF7", "chart_label": "🐼",
                        "logo": _PANDAS_SVG},
    "Python (Polars)": {"color": "#E47A2E", "bg": "#FEF5EC", "chart_label": "❄",
                        "logo": _POLARS_SVG},
    "Octave":          {"color": "#0F6680", "bg": "#E8F4F8", "chart_label": "Oct",
                        "logo": _OCTAVE_SVG},
...
LANG_MEMORY_NOTE = {
    "Python (NumPy)":  "tracemalloc (Python-managed heap)",
    "Python (Pandas)": "tracemalloc peak (Python-managed heap; includes DataFrame overhead)",
    "Python (Polars)": "tracemalloc peak (Python-managed heap; includes Polars Series/DataFrame overhead)",
    "Octave":          "output matrix size (theoretical; Octave allocates via BLAS/system malloc)",
...
FILE_MAP = {
    "Python (NumPy)":  "python_numpy_results.json",
    "Python (Pandas)": "python_pandas_results.json",
    "Python (Polars)": "python_polars_results.json",
    "Octave":          "octave_results.json",
    "R":       "r_results.json",
...
<div class="section">
  ...
  <ul class="chg-list">
    ...
    <li class="chg-item">
      ...
      <span class="chg-body">
        <span class="chg-tag chg-new">new</span>
        <strong>Added Python (Pandas), Python (Polars), and Octave</strong><br>
        Python is now split into three variants.
        <strong>Python (NumPy)</strong> is the original benchmark unchanged.
        <strong>Python (Pandas)</strong> wraps all operations in DataFrames/Series,
        using native <code>DataFrame @ DataFrame</code> matmul,
        <code>Series.dot()</code>, <code>Series.sort_values()</code>, and
        element-wise <code>df * df + df</code> for Hadamard;
        numpy/scipy handle decompositions via <code>.to_numpy()</code>.
        <strong>Python (Polars)</strong> uses Polars Series/DataFrames similarly —
        notably, Polars' native radix sort (<strong>~75 ms</strong>) is 6× faster
        than NumPy sort and 18× faster than Pandas sort for 10 M floats.
        <strong>Octave 11.1.0</strong> runs all 12 operations through its
        built-in BLAS/LAPACK, with JSON written without any package dependency.
      </span>
    </li>
    <li class="chg-item">
      <span class="chg-date">2026-04-25</span>
      <span class="chg-body">
        <span class="chg-tag chg-fix">fix</span>
        <strong>Rust FFT corrected: full complex FFT → real-to-complex FFT</strong><br>
        The Rust FFT benchmark was padding each real sample into a
        <code>Complex::new(x, 0.0)</code> value and running a full N-point complex
        FFT — roughly 2× the work of every other language, which all use a
        real-to-complex FFT producing N/2+1 outputs.
        Switched to the <code>realfft</code> crate (a thin wrapper around rustfft)
        for a proper real-to-complex transform; plan creation moved to setup so
        only the transform itself is timed.
        Corrected Rust FFT: <strong>~4 ms</strong> (was ~8 ms).
      </span>
    </li>
    <li class="chg-item">
      <span class="chg-date">2026-04-25</span>
      <span class="chg-body">
        <span class="chg-tag chg-fix">fix</span>
        <strong>SVD descriptions corrected from "Full" to "Economy"</strong><br>
        Every language was computing a thin/economy SVD
        (<code>full_matrices=False</code>, <code>thinSVD</code>,
        <code>ComputeThinU|ComputeThinV</code>, <code>dgesdd 'S'</code>,
        <code>svd 'econ'</code>) but all descriptions read "Full SVD".
        Updated to "Economy SVD" with explicit output shapes (U: m×min(m,n))
        across all 9 affected files.
      </span>
    </li>
    <li class="chg-item">
      <span class="chg-date">2026-04-25</span>
      <span class="chg-body">
        <span class="chg-tag chg-fix">fix</span>
        <strong>R and Octave FFT descriptions clarified</strong><br>
        Neither R nor Octave has a native real-to-complex FFT.
        Both call their full complex DFT (<code>fft()</code>) on real input,
        returning N complex outputs rather than the N/2+1 outputs produced
        by <code>rfft</code>-based implementations in other languages.
        Descriptions updated to reflect this, explaining the large gap
        (R: ~63 ms vs Python NumPy rfft: ~7 ms).
      </span>
    </li>
    <li class="chg-item">
...
<footer>
  ...
  v0.1 &nbsp;|&nbsp;
  <a href="https://github.com/kuod/math-comp-polyglot" target="_blank">Source on GitHub</a>
  &nbsp;|&nbsp; Fixed seed 42 &nbsp;|&nbsp;
